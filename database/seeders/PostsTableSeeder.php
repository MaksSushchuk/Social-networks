...
class PostsTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'text',
                'display_name' => __('voyager::seeders.data_rows.authors'),
                'required'     => 1,
                'browse'       => 0,
                'read'         => 1,
                'edit'         => 1,
        ...
        $dataRow = $this->dataRow($postDataType, 'text');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'rich_text_box',
                'display_name' => __('voyager::seeders.data_rows.text'),
                'required'     => 1,
                'browse'       => 0,
                'read'         => 1,
                'edit'         => 1,
        ...
        if (!$post->exists) {
            $post->fill([
                ...
                'text'             => '<p>This is the body of the lorem ipsum post</p>',
        ...
        if (!$post->exists) {
            $post->fill([
                ...
                'text'      => '<p>This is the body for the sample post, which includes the body.</p>
        ...
        if (!$post->exists) {
            $post->fill([
                ...
                'text'             => '<p>This is the body for the latest post</p>',
        ...
        if (!$post->exists) {
            $post->fill([
                ...
                'text'      => '<p>Swab deadlights Buccaneer fire ship square-rigged dance the hempen jig weigh anchor cackle fruit grog furl. Crack Jennys tea cup chase guns pressgang hearties spirits hogshead Gold Road six pounders fathom measured fer yer chains. Main sheet provost come about trysail barkadeer crimp scuttle mizzenmast brig plunder.</p>
